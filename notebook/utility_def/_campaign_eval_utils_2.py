...
def get_awareness(txn, cp_start_date, cp_end_date, store_fmt, test_store_sf, adj_prod_sf,
    ...
    (txn_exposed
     ...
     .agg(
       ...
       F.countDistinct( (F.when(F.col('household_id').isNotNull(), F.col('transaction_uid') ).otherwise(None) ) ).alias('carded_visits'),
       F.countDistinct( (F.when(F.col('household_id').isNull(), F.col('transaction_uid') ).otherwise(None) ) ).alias('non_carded_visits')
       )
...
def get_awareness_no_aisle(txn, 
   ...
   # .join(adj_prod_sf, 'upc_id', 'inner')
    ...
    (all_impression
     ...
     .withColumn('est_non_carded_reach', F.col('non_carded_visits')/F.col('avg_carded_freq') )
     .withColumn('total_reach', F.col('carded_reach') + F.col('est_non_carded_reach'))
     .withColumn('media_fee', F.lit(media_spend))
     .withColumn('CPM', F.lit(media_spend) / ( F.col('epos_impression') / 1000 ) )
    )         
...
# MAGIC %md ### Cust share by mech (dup control)

# COMMAND ----------

def cust_kpi_by_mech(txn
                    ,store_fmt
                    ,store_match_df
                    ,feat_df
                    ,brand_df
    ...
    features_subclass = sclass_df
    features_class    = class_df
    
    brand_list        = [brand[0] for brand in brand_df.select('brand_nm').dropDuplicates().collect()]
    
    test_store_sf     = store_match_df.select( store_match_df.store_id
                                              ,store_match_df.store_mech_set)\
                                      .dropDuplicates()
    
    ctl_df            = store_match_df.select( store_match_df.ctr_store_cos.alias('store_id')
                                              ,store_match_df.store_mech_set)\
                                      .dropDuplicates()
    ...
    txn_test_pre = txn_all.join(test_store_sf, 'store_id', 'inner').filter(F.col('period_fis_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('test'))
    txn_test_cmp = txn_all.join(test_store_sf, 'store_id', 'inner').filter(F.col('period_fis_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('test'))
    
    #txn_ctrl_pre = txn_all.filter(F.col('store_id').isin(ctr_store_list)).filter(F.col('period_fis_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    #txn_ctrl_cmp = txn_all.filter(F.col('store_id').isin(ctr_store_list)).filter(F.col('period_fis_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    
    txn_ctrl_pre = txn_all.join(ctl_df, 'store_id', 'inner').filter(F.col('period_fis_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    txn_ctrl_cmp = txn_all.join(ctl_df, 'store_id', 'inner').filter(F.col('period_fis_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    ...
    brand_in_subclass_kpi = \
    (txn_all_combine
     .join(features_subclass, 'upc_id', 'left_semi')
     .where(txn_all_combine.brand_name.isin(brand_list))     
     .groupBy('test_ctrl_store','period_fis_wk')
     .pivot('carded_nonCarded')
     .agg(*kpis)
     .withColumn('kpi_level', F.lit('feature_brand_in_subclass'))
    ...
    brand_in_class_kpi = \
    (txn_all_combine
     .join (features_class, 'upc_id', 'left_semi')
     .where(txn_all_combine.brand_name.isin(brand_list))
     .groupBy('test_ctrl_store','period_fis_wk')
     .pivot('carded_nonCarded')
     .agg(*kpis)
    ...
    df_pv = kpi_df[['test_ctrl_store', 'store_mech_set', 'period_fis_wk', 'kpi_level', 'MyLo_customer']].pivot(index=['test_ctrl_store','store_mech_set','period_fis_wk'], columns='kpi_level', values='MyLo_customer')
    df_pv['%cust_sku_in_subclass'] = df_pv['feature_sku']/df_pv['feature_subclass']*100
    df_pv['%cust_sku_in_class'] = df_pv['feature_sku']/df_pv['feature_class']*100
    df_pv['%cust_brand_in_subclass'] = df_pv['feature_brand_in_subclass']/df_pv['feature_subclass']*100
    df_pv['%cust_brand_in_class'] = df_pv['feature_brand_in_class']/df_pv['feature_class']*100
...
## Scent Add Cust kpi pre == post 
## 2024-03-21
def cust_kpi_by_mech_eq(txn
                    ,store_fmt
                    ,store_match_df
                    ,feat_df
                    ,brand_df
    ...
    #txn_test_pre = txn_all.join(test_store_sf.select('store_id').drop_duplicates() , 'store_id').filter(F.col('period_eq_fis_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('test'))
    #txn_test_cmp = txn_all.join(test_store_sf.select('store_id').drop_duplicates(), 'store_id').filter(F.col('period_eq_fis_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('test'))
    
    txn_test_pre = txn_all.join(test_store_sf, 'store_id', 'inner').filter(F.col('period_eq_fis_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('test'))
    txn_test_cmp = txn_all.join(test_store_sf, 'store_id', 'inner').filter(F.col('period_eq_fis_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('test'))
    
    #txn_ctrl_pre = txn_all.filter(F.col('store_id').isin(ctr_store_list)).filter(F.col('period_eq_fis_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    #txn_ctrl_cmp = txn_all.filter(F.col('store_id').isin(ctr_store_list)).filter(F.col('period_eq_fis_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    
    txn_ctrl_pre = txn_all.join(ctl_df, 'store_id', 'inner').filter(F.col('period_eq_fis_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    txn_ctrl_cmp = txn_all.join(ctl_df, 'store_id', 'inner').filter(F.col('period_eq_fis_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    ...
    (txn_all_combine
     .filter(F.col('upc_id').isin(feat_list))
     .groupBy('test_ctrl_store','period_eq_fis_wk')
     .pivot('carded_nonCarded')
     .agg(*kpis)
     .withColumn('kpi_level', F.lit('feature_sku'))
     .withColumn('store_mech_set', F.lit('ALL_MECH'))
    ...
    (txn_all_combine
     .filter(F.col('upc_id').isin(feat_list))
     .groupBy('test_ctrl_store','store_mech_set','period_eq_fis_wk')
     .pivot('carded_nonCarded')
     .agg(*kpis)
     .withColumn('kpi_level', F.lit('feature_sku'))
    )
    ...
    combined_kpi = sku_kpi.unionByName(brand_in_subclass_kpi)\
                          ...
                          .unionByName(brand_in_class_kpi_by_mech)\
                          .unionByName(all_class_kpi_by_mech)\
                          .unionByName(all_lotus_kpi_by_mech)\
                          .unionByName(all_lotus_kpi)\
    
    kpi_df = to_pandas(combined_kpi)
    
    df_pv = kpi_df[['test_ctrl_store', 'store_mech_set', 'period_eq_fis_wk', 'kpi_level', 'MyLo_customer']].pivot(index=['test_ctrl_store','store_mech_set','period_eq_fis_wk'], columns='kpi_level', values='MyLo_customer')
    df_pv['%cust_sku_in_subclass'] = df_pv['feature_sku']/df_pv['feature_subclass']*100
    df_pv['%cust_sku_in_class'] = df_pv['feature_sku']/df_pv['feature_class']*100
    df_pv['%cust_brand_in_subclass'] = df_pv['feature_brand_in_subclass']/df_pv['feature_subclass']*100
    df_pv['%cust_brand_in_class'] = df_pv['feature_brand_in_class']/df_pv['feature_class']*100
    df_pv['%cust_brand_in_lotus'] = df_pv['feature_brand_in_class']/df_pv['feature_lotus']*100
    df_pv.sort_index(axis=0, ascending=False, inplace=True)
    df_pv_out = df_pv.reset_index()
...
# MAGIC %md ### KPI Promo_week by mech

# COMMAND ----------

def cust_kpi_promo_wk_by_mech(txn
                    ,store_fmt
                    ,store_match_df
                    ,feat_df
                    ,brand_df
                    ...
                    ,class_df):
    """Full eval : customer KPIs Pre-Dur for test store vs control store
    - Features SKUs spark dataframe
    - Brand SKU spark dataframe (brand in category defintion)
    - All Feature subclass
    - Feature Brand in class
    - All Feature class
    """ 
    ...
    txn_test_pre = txn_all.join(test_store_sf, 'store_id', 'inner').filter(F.col('period_promo_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('test'))
    txn_test_cmp = txn_all.join(test_store_sf, 'store_id', 'inner').filter(F.col('period_promo_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('test'))
    
    #txn_ctrl_pre = txn_all.filter(F.col('store_id').isin(ctr_store_list)).filter(F.col('period_promo_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    #txn_ctrl_cmp = txn_all.filter(F.col('store_id').isin(ctr_store_list)).filter(F.col('period_promo_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    
    txn_ctrl_pre = txn_all.join(ctl_df, 'store_id', 'inner').filter(F.col('period_promo_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    txn_ctrl_cmp = txn_all.join(ctl_df, 'store_id', 'inner').filter(F.col('period_promo_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    ...
    txn_ctrl_combine = \
    (txn_ctrl_pre
     .unionByName(txn_ctrl_cmp, allowMissingColumns=True)
     .withColumn('carded_nonCarded', F.when(F.col('household_id').isNotNull(), 'MyLo').otherwise('nonMyLo'))
    )
    
    txn_all_combine = txn_test_combine.unionByName(txn_ctrl_combine, allowMissingColumns=True)    
    ...
    df_pv = kpi_df[['test_ctrl_store','store_mech_set', 'period_promo_wk', 'kpi_level', 'MyLo_customer']].pivot(index=['test_ctrl_store','store_mech_set','period_promo_wk'], columns='kpi_level', values='MyLo_customer')
    df_pv['%cust_sku_in_subclass'] = df_pv['feature_sku']/df_pv['feature_subclass']*100
    df_pv['%cust_sku_in_class'] = df_pv['feature_sku']/df_pv['feature_class']*100
    df_pv['%cust_brand_in_subclass'] = df_pv['feature_brand_in_subclass']/df_pv['feature_subclass']*100
    df_pv['%cust_brand_in_class'] = df_pv['feature_brand_in_class']/df_pv['feature_class']*100
...
## Scent Add Cust kpi pre == post 
## 2024-03-21
def cust_kpi_promo_wk_by_mech_eq(txn
                    ,store_fmt
                    ,store_match_df
                    ,feat_df
                    ,brand_df
    ...
    #txn_test_pre = txn_all.join(test_store_sf.select('store_id').drop_duplicates() , 'store_id').filter(F.col('period_eq_promo_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('test'))
    #txn_test_cmp = txn_all.join(test_store_sf.select('store_id').drop_duplicates(), 'store_id').filter(F.col('period_eq_promo_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('test'))
    
    txn_test_pre = txn_all.join(test_store_sf, 'store_id', 'inner').filter(F.col('period_eq_promo_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('test'))
    txn_test_cmp = txn_all.join(test_store_sf, 'store_id', 'inner').filter(F.col('period_eq_promo_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('test'))
    
    #txn_ctrl_pre = txn_all.filter(F.col('store_id').isin(ctr_store_list)).filter(F.col('period_eq_promo_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    #txn_ctrl_cmp = txn_all.filter(F.col('store_id').isin(ctr_store_list)).filter(F.col('period_eq_promo_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    
    txn_ctrl_pre = txn_all.join(ctl_df, 'store_id', 'inner').filter(F.col('period_eq_promo_wk').isin(['pre'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    txn_ctrl_cmp = txn_all.join(ctl_df, 'store_id', 'inner').filter(F.col('period_eq_promo_wk').isin(['cmp'])).withColumn('test_ctrl_store', F.lit('ctrl'))
    ...
    (txn_all_combine
     .filter(F.col('upc_id').isin(feat_list))
     .groupBy('test_ctrl_store','period_eq_promo_wk')
     .pivot('carded_nonCarded')
     .agg(*kpis)
     .withColumn('kpi_level', F.lit('feature_sku'))
     .withColumn('store_mech_set', F.lit('ALL_MECH'))
    ...
    (txn_all_combine
     .filter(F.col('upc_id').isin(feat_list))
     .groupBy('test_ctrl_store','store_mech_set','period_eq_promo_wk')
     .pivot('carded_nonCarded')
     .agg(*kpis)
     .withColumn('kpi_level', F.lit('feature_sku'))
    )
    ...
    combined_kpi = sku_kpi.unionByName(brand_in_subclass_kpi, allowMissingColumns=True)\
                          ...
                          .unionByName(all_lotus_kpi_by_mech , allowMissingColumns=True)
    
    
    kpi_df = to_pandas(combined_kpi)
    
    df_pv = kpi_df[['test_ctrl_store','store_mech_set', 'period_eq_promo_wk', 'kpi_level', 'MyLo_customer']].pivot(index=['test_ctrl_store','store_mech_set','period_eq_promo_wk'], columns='kpi_level', values='MyLo_customer')
    df_pv['%cust_sku_in_subclass'] = df_pv['feature_sku']/df_pv['feature_subclass']*100
    df_pv['%cust_sku_in_class'] = df_pv['feature_sku']/df_pv['feature_class']*100
    df_pv['%cust_brand_in_subclass'] = df_pv['feature_brand_in_subclass']/df_pv['feature_subclass']*100
    df_pv['%cust_brand_in_class'] = df_pv['feature_brand_in_class']/df_pv['feature_class']*100
...
## > Expect to have "store_region" in store matching table already
## ---------------------------------------------------------------
## Danny add CC only sales uplift , % sales uplift
## Date : 9 July 2023
## CC only uplift to check alignment with Customer uplift
##----------------------------------------------------------------------------
...
def sales_uplift_reg_mech(txn, 
    ...
    try:
        ...
        matching_df_2 = matching_df[['store_id','test_store_region', 'store_mech_set', 'ctr_store_cos']].copy()  ## Pat Add store_region - May 2022
        matching_df_2.rename(columns={'store_id':'store_id_test','ctr_store_cos':'store_id_ctr', 'test_store_region':'store_region', 'store_mech_set':'store_mech_set'},inplace=True)
...
#    cc_sales_pre_df = to_pandas(cc_sales_pre)
    ...
    if sum_sales_ctrl_adj == 0:
        sum_sales_ctrl_adj = np.nan
    ## end if
    ...
    if sum_cc_sales_ctrl_adj == 0:
        sum_cc_sales_ctrl_adj = np.nan
    ## end if
