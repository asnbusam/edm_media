...
lyt_desc = spark.createDataFrame(
    [("premium", 1), ("valuable", 2), ("potential", 3), ("uncommitted", 4)],
...
def _map_format_channel(txn: SparkDataFrame):
    ...
    mapped_chan_txn = (
        txn.withColumn("channel", F.trim(F.col("channel")))
        .withColumn(
            "channel_group_forlotuss",
            F.when(F.col("channel") == "OFFLINE", "OFFLINE")
            .when(
                F.col("channel").isin(["Click and Collect", "HATO", "Scheduled CC"]),
                "Click and Collect + HATO",
            )
            .when(
                F.col("channel").isin(
                    [
        ...
        .withColumn(
            "offline_online_other_channel",
            F.when(
                F.col("channel_group_forlotuss") == "OFFLINE", F.lit("OFFLINE")
            ).otherwise(F.lit("ONLINE")),
        )
        .withColumn(
            "store_format_online_subchannel_other",
            F.when(
                F.col("offline_online_other_channel") == "OFFLINE",
                F.col("store_format_group"),
...
def get_trans_itm(
    ...
    if use_business_date:
        item: SparkDataFrame = item.withColumn(
            "business_date", F.coalesce(F.to_date("business_date"), F.col("date_id"))
    ...
    # ---- if not use business date
        ...
        item: SparkDataFrame = item.filter(
            F.col("date_id").between(str_date_id, end_date_id)
        )
...
def get_trans_itm_wkly_promo(
    ...
    item: SparkDataFrame = spark.table(TBL_ITEM).join(data_period, "date_id", "inner")

    ## add fitler basket using promoweek_id

    bask: SparkDataFrame = spark.table(TBL_BASK).join(data_period, "date_id", "inner")
    ...
    def wrapper(*args, **kwargs):
        ...
        adjusted_store_region = (
            ...
            .withColumn(
                "store_region",
                F.when(
                    F.col("store_region").isin(["West", "Central"]),
                    F.lit("West+Central"),
...
def sparkDataFrame_to_csv_filestore(
    sf: SparkDataFrame, csv_folder_name: str, prefix: str
) -> bool:
    ...
    html_url = os.path.join(
        "https://adb-2606104010931262.2.azuredatabricks.net/files",
        filestore_path_to_url,
        csv_folder_name + "?o=2606104010931262",
    )
...
def to_pandas(sf: SparkDataFrame) -> PandasDataFrame:
    ...
    select_cast_exp = [
        F.col(c[0]).cast(T.DoubleType()) if c[1].startswith("decimal") else F.col(c[0])
        for c in col_name_type
    ]
...
def pandas_to_csv_filestore(
    df: PandasDataFrame, csv_file_name: str, prefix: str
) -> str:
    ...
    try:
        ...
        html_url = os.path.join(
            "https://adb-2606104010931262.2.azuredatabricks.net/files",
            filestore_path_to_url + "?o=2606104010931262",
        )
        print(f"HTML download link :\n")
        return html_url
...
def get_vector_numpy(sf: SparkDataFrame, vec_col: str) -> numpyNDArray:
    ...
    out_np = (
        (
            sf.withColumn(vec_export_name, vector_to_array(vec_col)).select(
                [F.col(vec_export_name)[i] for i in range(v_size)]
            )
        )
        .toPandas()
        .to_numpy()
...
def bucket_bin(sf: SparkDataFrame, splits: List, split_col: str) -> SparkDataFrame:
    ...
    bin_desc_list = [
        [i, "[" + str(l) + " - " + str(w) + ")"]
        for i, (l, w) in enumerate(zip(low_bound, up_bound))
    ]
...
def create_zip_from_dbsf_prefix(dbfs_prefix_to_zip: str, output_zipfile_name: str):
    ...
    source_filenames = [
        ...
        for root, dirs, files in os.walk(top=source_dbfs_prefix, topdown=False)
        for name in files
    ]
    ...
    # ---- HTML direct download link
    root_dbfs_prefix_to_zip_remove_first_2_parts = str(
        Path(*Path(root_dbfs_prefix_to_zip).parts[3:])
    )
    print(root_dbfs_prefix_to_zip)
    html_url = os.path.join(
        "https://adb-2606104010931262.2.azuredatabricks.net/files",
        root_dbfs_prefix_to_zip_remove_first_2_parts,
        output_zipfile_name + "?o=2606104010931262",
    )
    print(f"HTML download link :\n{html_url}")
...
def create_zip_from_dbsf_prefix_indir(
    dbfs_prefix_to_zip: str, output_zipfile_name: str
):
    """
    Create zip from all file in dbfs prefix and show download link
    The zip file will be written to driver then copy back to dbsf for download url generation
    ...
    dbfs_prefix_to_zip_remove_first_2_parts = str(
        Path(*Path(dbfs_prefix_to_zip).parts[3:])
    )
    ...
    html_url = os.path.join(
        "https://adb-2606104010931262.2.azuredatabricks.net/files",
        dbfs_prefix_to_zip_remove_first_2_parts,
        output_zipfile_name + "?o=2606104010931262",
    )
    print(f"HTML download link :\n{html_url}")
...
def get_dir_content(ls_path):
    ...
    subdir_paths = [
        get_dir_content(p.path) for p in dir_paths if p.isDir() and p.path != ls_path
    ]
