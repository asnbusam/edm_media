...
# MAGIC %md ## Get prod df custom SKU (for adhoc version only) -- Pat 3 JUl 2024

# COMMAND ----------

def _get_prod_df_customsku( sku_list: List
                          , cate_lvl: str
                          , std_ai_df : sparkDataFrame
                          , x_cate_flag
                          , x_cate_cd:str 
                          , c_sku_df: sparkDataFrame):
    """To get Product information refering to input SKU list (expected input as list )
       function will return feature dataframe (feat_df)
                          , brand dataframe (brand_df) 
                          , subclass dataframe (sclass_df) containing all product in subclass for matching
    ...
    ## 19 Jun 2024 : add ontop logic to customize product table if needed. 
    """
    prod_c   = sqlContext.table('tdm.v_prod_dim_c')
    mfr      = sqlContext.table('tdm.v_mfr_dim_c')
    
    prod_all = prod_c.join  (c_sku_df, prod_c.upc_id == c_sku_df.upc_id, 'left')\
                     .select( prod_c.upc_id
                             ,prod_c.mfr_id
                             ,F.when(c_sku_df.prod_desc.isNotNull(), c_sku_df.prod_desc)
                               .otherwise(prod_c.product_en_desc)
                               .alias('product_en_desc')
                             ,F.when(c_sku_df.custom_brand_name.isNotNull(), c_sku_df.custom_brand_name)
                               .otherwise(prod_c.brand_name)
                               .alias('brand_name')
                             ,F.when(c_sku_df.custom_division_code.isNotNull(), c_sku_df.custom_division_code)
                               .otherwise(prod_c.division_id)
                               .alias('division_id')
                             ,F.when(c_sku_df.custom_division_name.isNotNull(), c_sku_df.custom_division_name)
                               .otherwise(prod_c.division_name)
                               .alias('division_name')                            
                             ,F.when(c_sku_df.custom_department_code.isNotNull(), c_sku_df.custom_department_code)
                               .otherwise(prod_c.department_code)
                               .alias('department_code')
                             ,F.when(c_sku_df.custom_department_name.isNotNull(), c_sku_df.custom_department_name)
                               .otherwise(prod_c.department_name)
                               .alias('department_name')
                             ,F.when(c_sku_df.custom_section_code.isNotNull(), c_sku_df.custom_section_code)
                               .otherwise(prod_c.section_code)
                               .alias('section_code')
                             ,F.when(c_sku_df.custom_section_name.isNotNull(), c_sku_df.custom_section_name)
                               .otherwise(prod_c.section_name)
                               .alias('section_name')
                             ,F.when(c_sku_df.custom_class_code.isNotNull(), c_sku_df.custom_class_code)
                               .otherwise(prod_c.class_code)
                               .alias('class_code')
                             ,F.when(c_sku_df.custom_class_name.isNotNull(), c_sku_df.custom_class_name)
                               .otherwise(prod_c.class_name)
                               .alias('class_name')
                             ,F.when(c_sku_df.custom_subclass_code.isNotNull(), c_sku_df.custom_subclass_code)
                               .otherwise(prod_c.subclass_code)
                               .alias('subclass_code')
                             ,F.when(c_sku_df.custom_subclass_name.isNotNull(), c_sku_df.custom_subclass_name)
                               .otherwise(prod_c.subclass_name)
                               .alias('subclass_name')
                            )
