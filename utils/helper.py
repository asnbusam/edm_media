...
def get_vector_numpy(sf: SparkDataFrame, vec_col: str) -> numpyNDArray:
    ...
    out_np = (
        (
            sf.withColumn(
                vec_export_name, vector_to_array(vec_col)
            ).select(  # type: ignore
...
def get_lag_id(
    epoch_id: str, lag_num: int, full_year_count: int, inclusive: bool = True
):
    ...
    if remainder_part <= lag_num:
        ...
        diff_remainder_num = (
            math.ceil(lag_num / full_year_count) * full_year_count - lag_num
        )
...
def get_lag_period_id(pr_id: str, lag_num: int, inclusive: bool = True):
    """Get lagging priod_id"""
    lag_pr_id = get_lag_id(
        epoch_id=pr_id, lag_num=lag_num, full_year_count=12, inclusive=inclusive
    )
    return lag_pr_id


def get_lag_wk_id(wk_id: str, lag_num: int, inclusive: bool = True):
    """Get lagging week_id"""
    lag_wk_id = get_lag_id(
        epoch_id=wk_id, lag_num=lag_num, full_year_count=52, inclusive=inclusive
    )
    return lag_wk_id
